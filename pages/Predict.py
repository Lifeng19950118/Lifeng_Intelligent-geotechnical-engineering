...
st.sidebar.markdown("# Proxy Model 🧪")
...
ax2.set_xlabel('Numerical simulation result')
ax2.set_ylabel('Predictions')
ax2.set_title('Numerical simulation result vs Predicted')

c11, c12= st.columns(2)
c41, c42= st.columns(2)
c61, c62,c63= st.columns([9,1,1])
c71,c72= st.columns([1,1])
c31, c32, c33, c34= st.columns([1.5,1.5,1,1])
...
with c41:
   st.table(enbdf.head(4))
df=enbdf_n
df_copy=df.copy()
with c42:
   st.table(df_copy.head(4))
...
uploaded_file = st.file_uploader("Select a file (.csv)",label_visibility="hidden")
with c61:
...
# =============================================================================
    ...
    with c71:
        st.download_button(
            label="Download stander data as CSV",
            data=csv,
    ...
    with c72:
        st.download_button(
            label="Download unstander data as CSV",
            data=csv,
